...
def load_db_metadata(meta_path, n_entries):
    if meta_path and Path(meta_path).exists():
        ...
        if len(df) != n_entries:
            logging.warning(f"metadata rows ({len(df)}) != database entries ({n_entries}), using synthetic IDs")
...
class RAGRetriever:
    def __init__(self, database_path, db_meta_path=None,
                 top_k=5, query_weight=0.5,
                 maxseq=35, emb_dim=1280, metric="cosine"):
        self.top_k        = top_k
    ...
    def retrieve(self, query, return_metadata=False):
        ...
        if q.ndim == 1:
            ...
            raise ValueError(f"query shape {q.shape} does not match expected ({self.maxseq}, {self.emb_dim})")
    ...
    def retrieve_batch(self, queries, batch_size=100):
        ...
        for b in range(n_batches):
            ...
            logging.info(f"[{b+1}/{n_batches}] batch done ({start}-{end-1})")
        return fused

    def discriminate(self, embeddings, threshold=0.5, return_scores=False):
        if embeddings.ndim == 4:
            embeddings = embeddings.reshape(embeddings.shape[0], self.maxseq, self.emb_dim)
        ...
        logging.info(f"{mask.sum()}/{len(mask)} embeddings passed (threshold={threshold:.3f})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--metric",         default="cosine", choices=["l2", "cosine"])
    ...
    if args.discriminate:
        mask, scores = rag.discriminate(queries, threshold=args.disc_threshold, return_scores=True)
        logging.info(f"first 10 scores: {scores[:10].round(4)}")
        filtered = queries[mask]
        out_path = Path(args.output)
        np.save(str(out_path.with_stem(out_path.stem + "_filtered")),
